...
/**
 * The overlay layers, in draw order.
 *
 ...
 *
 * They are installed at the visibility the document asks for rather than hidden
 * and then corrected. A course saved with contours on should open with contours
 * on, not open bare and grow them a frame later.
 */
  ...
  return [
    {
      ...
      type: 'hillshade',
      source: DEM_SOURCE,
      layout: { visibility: shown(overlays.hillshade) },
      paint: {
        /*
         ...
         * shadowed side and leaves flat ground alone, so tree canopy still
         * looks like tree canopy and the relief arrives as a separate signal.
         */
    ...
    },
    {
      ...
      layout: { visibility: shown(overlays.contours), 'line-join': 'round' },
      paint: {
    ...
    },
    {
      ...
      filter: ['>', ['get', 'level'], 0],
      layout: {
        visibility: shown(overlays.contours),
